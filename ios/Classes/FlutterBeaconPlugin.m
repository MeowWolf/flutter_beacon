...
#define rangingStatusTimeInterval 4
@interface FlutterBeaconPlugin () <CLLocationManagerDelegate,
								   CBCentralManagerDelegate,
								   CBPeripheralManagerDelegate>

@property(assign, nonatomic)
	CLAuthorizationStatus defaultLocationAuthorizationType;
...
@property(strong, nonatomic) FBRangingStatusStreamHandler *rangingStatusHandler;
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar> *)registrar {
  FlutterMethodChannel *channel =
	  [FlutterMethodChannel methodChannelWithName:@"flutter_beacon"
								  binaryMessenger:[registrar messenger]];
  FlutterBeaconPlugin *instance = [[FlutterBeaconPlugin alloc] init];
  [registrar addMethodCallDelegate:instance channel:channel];

  instance.rangingHandler =
	  [[FBRangingStreamHandler alloc] initWithFlutterBeaconPlugin:instance];
  ...
  instance.monitoringHandler =
	  [[FBMonitoringStreamHandler alloc] initWithFlutterBeaconPlugin:instance];
  FlutterEventChannel *streamChannelMonitoring = [FlutterEventChannel
	  eventChannelWithName:@"flutter_beacon_event_monitoring"
		   binaryMessenger:[registrar messenger]];
  [streamChannelMonitoring setStreamHandler:instance.monitoringHandler];

  instance.bluetoothHandler =
	  [[FBBluetoothStateHandler alloc] initWithFlutterBeaconPlugin:instance];
  FlutterEventChannel *streamChannelBluetooth = [FlutterEventChannel
	  eventChannelWithName:@"flutter_bluetooth_state_changed"
		   binaryMessenger:[registrar messenger]];
  [streamChannelBluetooth setStreamHandler:instance.bluetoothHandler];

  instance.authorizationHandler = [[FBAuthorizationStatusHandler alloc]
	  initWithFlutterBeaconPlugin:instance];

  FlutterEventChannel *streamChannelAuthorization = [FlutterEventChannel
	  eventChannelWithName:@"flutter_authorization_status_changed"
		   binaryMessenger:[registrar messenger]];
  [streamChannelAuthorization setStreamHandler:instance.authorizationHandler];


  instance.rangingStatusHandler = [[FBRangingStatusStreamHandler alloc]
		initWithFlutterBeaconPlugin:instance];
...
- (id)init {
  ...
  if (self) {
	...
	self.defaultLocationAuthorizationType =
...
- (void)handleMethodCall:(FlutterMethodCall *)call
  ...
  if ([@"setLocationAuthorizationTypeDefault" isEqualToString:call.method]) {
	if (call.arguments != nil &&
		[call.arguments isKindOfClass:[NSString class]]) {
  ...
  if ([@"openBluetoothSettings" isEqualToString:call.method]) {
	...
	// Beware, this is considered as a private API and Apple will rejecte your
	// application Uncomment these codes below if your want to publish this app
	// privately
	/*
	NSString *settingsUrl= @"App-Prefs:root=Bluetooth";
	if ([[UIApplication sharedApplication]
	respondsToSelector:@selector(openURL:options:completionHandler:)]) { if
	(@available(iOS 10.0, *)) {
			[[UIApplication sharedApplication] openURL:[NSURL
	URLWithString:settingsUrl] options:@{} completionHandler:^(BOOL success) {
				NSLog(@"Bluetooth settings opened");
			}];
		} else {
			[[UIApplication sharedApplication] openURL:[NSURL
	URLWithString:settingsUrl]];
		}
	}
	 */
  ...
  if ([@"openLocationSettings" isEqualToString:call.method]) {
	...
	// Beware, this is considered as a private API and Apple will reject your
	// application Uncomment these codes below if your want to publish this app
	// privately
	/*
...
- (void)initializeRangingStatusTimer {
  if (!self.timer) {
	self.timer =
		[NSTimer scheduledTimerWithTimeInterval:rangingStatusTimeInterval
										 target:self
									   selector:@selector(handleRangingStatus:)
									   userInfo:nil
										repeats:YES];
  }
}
...
- (void)startBroadcast:(id)arguments {
  ...
  self.beaconPeripheralData =
	  [region peripheralDataWithMeasuredPower:measuredPower];
...
- (void)initializeWithResult:(FlutterResult)result {
  ...
  [self initializeLocationManager];
  [self initializeCentralManager];
  [self initializeRangingStatusTimer];
}
...
- (void)centralManagerDidUpdateState:(CBCentralManager *)central {
  ...
  if (self.flutterResult) {
	if (message) {
	  self.flutterResult([FlutterError errorWithCode:@"Beacon"
											 message:message
											 details:nil]);
	} else {
...
- (void)locationManager:(CLLocationManager *)manager
  ...
  if (self.flutterEventSinkRanging) {
	...
	self.flutterEventSinkRanging(@{@"region" : dictRegion, @"beacons" : array});
  }
}
  ...
  if (self.flutterEventSinkMonitoring) {
	...
	if (reg) {
	  ...
	  self.flutterEventSinkMonitoring(
		  @{@"event" : @"didEnterRegion", @"region" : dictRegion});
	}
  }
}
...
- (void)locationManager:(CLLocationManager *)manager
  ...
  if (self.flutterEventSinkMonitoring) {
	...
	if (reg) {
	  NSDictionary *dictRegion = [FBUtils dictionaryFromCLBeaconRegion:reg];
	  self.flutterEventSinkMonitoring(
		  @{@"event" : @"didExitRegion", @"region" : dictRegion});
	}
  }
}
...
- (void)peripheralManagerDidStartAdvertising:(CBPeripheralManager *)peripheral
  ...
  if (error) {
	self.flutterBroadcastResult([FlutterError
		errorWithCode:@"Broadcast"
...
//------------------------------------------------------------
#pragma mark - Ranging Status Handler
///------------------------------------------------------------

- (void)handleRangingStatus:(NSTimer *)timer {
  if(self.flutterEventSinkRangingStatus) {
	BOOL isRanging = [self.locationManager.rangedRegions count] > 0;
	self.flutterEventSinkRangingStatus(
			@{@"ranging_status" : [NSNumber numberWithBool:isRanging]});
  }
